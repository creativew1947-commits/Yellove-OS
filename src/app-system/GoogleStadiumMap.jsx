...
import { drawRoute, getTransitConstants, getMapConstants } from '../core/utils';
...
const GoogleStadiumMap = ({ isLoaded = false, origin = null, destination = null, waypoints = [], markers = [], externalDirections = null, matchMode = false }) => {
  ...
  const calculateEtas = (orig, dest) => {
    ...
    // Strategic Congestion Factor: 50% increase during Match Mode
    const congestionFactor = matchMode ? 1.5 : 1.0;

    setEtas({
      taxi: Math.max(5, Math.ceil(distanceKm * 2.5 * congestionFactor)),
      metro: Math.max(8, Math.ceil(distanceKm * 1.8 * congestionFactor)),
      train: Math.max(12, Math.ceil(distanceKm * 2.0 * congestionFactor)),
      bus: Math.max(15, Math.ceil(distanceKm * 4.0 * congestionFactor))
    });
  };
    ...
    <div className="relative w-full h-full">
        ...
        >
          ...
          {transitMarkers.map(tm => (
            <Marker
                key={tm.id}
                title={`Transit: ${tm.name} at ${tm.stop}`}
        ...
        {destination && etas && (() => {
            const modes = [
                ...
                { id: 'metro', name: 'Metro', link: 'https://tickets.chennaimetrorail.org/onlineticket', icon: 'fa-subway' },
